// app/api/estimated-tax-payments/route.ts

import { NextResponse, type NextRequest } from "next/server";
import { z } from "zod";
import { db } from "@/drizzle/db";
import { estimatedTaxPayments } from "@/drizzle/schema";
...
const dateField = z
  .string()
  .default("")
  .refine((v) => isIsoDateOrEmpty(v), { message: "Use a valid date (YYYY-MM-DD)" });
...
const Schema = z
  ...
  .superRefine((d, ctx) => {
    ...
    for (const [dateKey, amtKey, label] of pairs) {
      ...
      if (hasAmt && !hasDate) {
        ctx.addIssue({ code: "custom", path: [dateKey], message: `${label}: date is required when amount is entered` });
      }
      if (hasDate && !hasAmt) {
        ctx.addIssue({ code: "custom", path: [amtKey], message: `${label}: amount is required when date is entered` });
      }
    }
  });
...
export async function GET() {
  ...
  const [row] = await db
    .select({ data: estimatedTaxPayments.data })
    .from(estimatedTaxPayments)
    .where(eq(estimatedTaxPayments.userId, user.id))
    .limit(1);
...
export async function POST(req: NextRequest) {
  ...
  await db
    .insert(estimatedTaxPayments)
    .values({ userId: user.id, data: parsed.data, updatedAt: new Date() })
    .onConflictDoUpdate({
      target: estimatedTaxPayments.userId,
