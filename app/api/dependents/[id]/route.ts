...
export async function PATCH(
  ...
  if (typeof b.firstName === "string") {
    set.firstName = b.firstName.trim().slice(0, 80);
  }
  if (typeof b.lastName === "string") {
    set.lastName = b.lastName.trim().slice(0, 80);
  }
  if (typeof b.dob === "string") {
    set.dob = b.dob.trim().slice(0, 10);
  }
  if (typeof b.relationship === "string") {
    set.relationship = b.relationship.trim().slice(0, 40);
  }
  if (typeof b.ssnLast4 === "string") {
    const s = b.ssnLast4.replace(/\D/g, "").slice(0, 4);
    if (s.length !== 4) {
      return NextResponse.json(
        { error: "Invalid ssnLast4" },
        { status: 400 }
      );
    }
    set.ssnLast4 = s;
