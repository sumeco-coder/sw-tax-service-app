...
// returns "" if invalid, otherwise YYYY-MM-DD
...
export async function saveQuestions(formData: FormData) {
  ...
  // ✅ validate + encrypt dependents (IMPORTANT: middleName must be STRING because schema is .notNull())
  const dependentRows = dependentInputs.map((d, idx) => {
    const firstName = clean(d.firstName, 60);
    const middleName = clean(d.middleName ?? "", 60); // ✅ always string ("" allowed)
    ...
    return {
      ...
      middleName, // ✅ string
      lastName,
      dob, // ✅ YYYY-MM-DD string (works with pg date column)
      relationship,
  ...
  await db
    ...
    .onConflictDoUpdate({
      ...
      set: { email, onboardingStep: "QUESTIONS" as any, updatedAt: new Date() },
    });
    ...
    if (dependentRows.length) {
      await tx.insert(dependents).values(
        dependentRows.map((r) => ({
          ...
          middleName: r.middleName, // ✅ string (no null)
          lastName: r.lastName,
          dob: r.dob, // ✅ string YYYY-MM-DD
          relationship: r.relationship,
          ssnEncrypted: r.ssnEncrypted,
          // monthsInHome / isStudent / isDisabled use DB defaults
        }))
      );
    }
  });
