...
/**
 * ✅ OPTIONAL: SKIP SCHEDULING
 * IMPORTANT: does NOT submit onboarding and does NOT go to /profile.
 * It sends them to SUMMARY so they can proceed to AGREEMENTS.
 */
export async function skipScheduling(): Promise<void> {
  ...
  // ✅ Schedule is optional. Move them forward to SUMMARY (then Agreements).
  await db
    .update(users)
    .set({
...
/**
 * BOOK APPOINTMENT (optional step)
 * After booking, send them to SUMMARY (then Agreements) — NOT /profile.
 *
 * Expects form fields:
...
export async function bookAppointment(formData: FormData): Promise<void> {
  ...
  // ✅ Prevent duplicates: if they already have a scheduled appt, update it
  const [existingAppt] = await db
    .select()
    .from(appointments)
    .where(
      and(
        eq(appointments.userId, userRow.id),
  ...
  // ✅ Booking schedule does NOT mean onboarding is done.
  // Move them to SUMMARY so they can sign AGREEMENTS next.
  await db
    .update(users)
    .set({
      onboardingStep: "SUMMARY",
 ...
 * CANCEL APPOINTMENT
 * Security:
 * - only owner (cookie sub) can cancel (or admin)
 */
 ...
 * Security:
 * - only owner (cookie sub) can reschedule (or admin)
 */
