// app/(client)/questionnaire/_components/EstimatedFederalTaxPayments.tsx
"use client";
...
import { PiggyBank, CalendarDays, DollarSign } from "lucide-react";
...
const schema = z
  ...
  .superRefine((d, ctx) => {
    // if user enters amount, require date (and vice versa)
...
export type EstimatedFederalTaxPaymentsValues = z.infer<typeof schema>;

const defaultValues: EstimatedFederalTaxPaymentsValues = {
...
function QuarterRow({
  ...
  title: string;
  dateName: keyof EstimatedFederalTaxPaymentsValues;
  amountName: keyof EstimatedFederalTaxPaymentsValues;
  register: any;
  setValue: any;
  errors: any;
}) {
...
export default function EstimatedFederalTaxPayments({
  initialValues,
  onSave,
  onBack,
  saveLabel = "Save",
}: {
  initialValues?: Partial<EstimatedFederalTaxPaymentsValues>;
  onSave?: (values: EstimatedFederalTaxPaymentsValues) => Promise<void> | void;
  onBack?: () => void;
  saveLabel?: string;
}) {
  const [msg, setMsg] = React.useState("");

  // ✅ cast fixes the “Resolver<optional> not assignable to Resolver<required>” error
  const resolver: Resolver<EstimatedFederalTaxPaymentsValues> =
    zodResolver(schema) as unknown as Resolver<EstimatedFederalTaxPaymentsValues>;

  const form = useForm<EstimatedFederalTaxPaymentsValues>({
    resolver,
  ...
  const submit: SubmitHandler<EstimatedFederalTaxPaymentsValues> = async (values) => {
    setMsg("");
    ...
    // default endpoint (change if you want)
    ...
    <Card className="border-slate-200">
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          <div>
            <CardTitle className="text-slate-900">Estimated Federal Tax Payments</CardTitle>
            <div className="mt-2 h-1 w-28 rounded-full" style={brandGradient} />
            <p className="mt-3 text-sm text-slate-600">
              Enter estimated tax payments already made for this year (optional).
            </p>
          </div>
