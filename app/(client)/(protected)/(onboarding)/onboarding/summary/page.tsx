...
async function continueToAgreements(_formData: FormData) {
  ...
  // ✅ NEW FLOW:
  // - SUBMITTED => show /onboarding/complete
  // - DONE => go dashboard
...
export default async function OnboardingSummaryPage() {
  ...
  const isSubmitted = onboardingStep === "SUBMITTED";
  const isDone = onboardingStep === "DONE";
  const isFinalized = isSubmitted || isDone;
  ...
  const sections: Section[] = sectionConfigs.map((cfg) => {
    ...
    if (isFinalized) status = "complete";
  ...
  const readyToContinue = requiredSections.every((s) => s.status === "complete") && !isFinalized;
  ...
  const statusTone =
    isDone || isSubmitted || alreadyInAgreements || readyToContinue
      ? "text-emerald-200"
      : "text-amber-200";
          ...
          <section className="space-y-4">
            ...
            >
              ...
              <p className="mt-1 text-xs text-white/85">
                {isDone
                  ? "You’re all set. You can manage everything from your dashboard."
                  : isSubmitted
                  ? "We received your onboarding. Finish one last step and then go to your dashboard."
                  : alreadyInAgreements
              ...
              <div className="mt-4 space-y-2">
                ...
                <p className="text-[11px] text-white/80 text-center">
                  {isDone
                    ? "You can update info anytime."
                    : isSubmitted
                    ? "Next: one last confirmation screen."
