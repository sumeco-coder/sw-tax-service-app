...
export default function DashboardPage() {
 ...
 const messageInputRef = useRef<HTMLInputElement>(null!);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [t, d, i, m, s, o, a, ys] = await Promise.all([
        ...
        apiGet<YearSnapshot[]>(`/api/tax-years`, []), // NEW endpoint
      ]);
  ...
  /* NEW: derived onboarding helpers */
  const currentStepIndex = useMemo(() => {
    const idx = ONBOARDING_FLOW.findIndex((s) => s.id === onboardingStep);
    return idx === -1 ? 0 : idx;
  }, [onboardingStep]);
  ...
  const currentStepConfig = ONBOARDING_FLOW[currentStepIndex];
  const isDone = onboardingStep === "DONE";

  // --- Return timeline derived state ---
  const statusStageIndex = React.useMemo(
