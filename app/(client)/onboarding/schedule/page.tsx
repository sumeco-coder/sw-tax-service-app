...
export default function SchedulePage() {
  ...
  // TODO: replace this with real data from your DB / server component
  ...
  const [selectedSlot, setSelectedSlot] =
    useState<(typeof TIME_SLOTS)[number] | null>(null);
  ...
  const scheduledAtIso =
    selectedDate && selectedSlot
      ? buildIsoDateTime(selectedDate, selectedSlot.value)
      : "";
        ...
        {dates.map((date) => {
          ...
          return (
            <button
              ...
              className={`rounded-xl p-3 text-center shadow-sm border transition
                ${
                  isSelected
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 px-4 py-10">
      <div className="mx-auto max-w-2xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        ...
        <header className="mb-6">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Pick a time for your tax professional to go over your return and
            next steps.
          </p>
        </header>
        ...
        {!currentAppointment && (
          <form action={bookAppointment} className="space-y-6">
            {/* Hidden identity + appointment fields */}
